...
class MoonAreas extends Component {
  render() {
    return (
      <div>
          ...
          <g className="areas areas-moon" fill="none">
            {paths.map((p) => {
              if (
                ...
                ["29", "30", "31"].includes(p.label)
              ) {
                return <path className="invisible" key={p.id} id={p.id} d={p.d} />;
              } else if (
                ...
                return <path key={p.id} id={p.id} d={p.d} />;
              }
            })}
          </g>
        </svg>
